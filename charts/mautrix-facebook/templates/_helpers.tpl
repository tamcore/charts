...
*/}}
{{- define "mautrix-facebook.configEnv.secretName" -}}
{{- if .Values.configEnv.existingSecret -}}
{{- .Values.configEnv.existingSecret -}}
{{- else -}}
{{- include "mautrix-facebook.fullname" . }}-config-env
{{- end -}}
{{- end -}}
...
*/}}
{{- define "mautrix-facebook.configEnv.validate" -}}
{{- if .Values.configEnv.enabled -}}
  {{- if and (not .Values.configEnv.existingSecret) (not .Values.configEnv.secretEnv) -}}
    {{- fail "mautrix-facebook: configEnv.enabled requires either configEnv.existingSecret or configEnv.secretEnv" -}}
  {{- end -}}
  {{- if not .Values.configEnv.variables -}}
    {{- fail "mautrix-facebook: configEnv.enabled requires configEnv.variables to be non-empty" -}}
  {{- end -}}
{{- end -}}
{{- end -}}
